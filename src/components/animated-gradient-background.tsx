...
export const AnimatedGradientBackground: FC = () => {
  ...
  useEffect(() => {
    // Skip animation entirely on low-end devices or when animations should be reduced
    if (DEVICE_CAPS.isLowEnd || DEVICE_CAPS.shouldReduceAnimations) {
      // Set static background for low-end devices
    ...
    const animate = (): void => {
      ...
      const updateFrequency = DEVICE_CAPS.isMobile ? 3 : 1; // Update every 3rd frame on mobile
      ...
      if (DEVICE_CAPS.isMobile) {
        // Simple gradient for mobile devices
        const baseGray = Math.round(35 + Math.sin(elapsed * 0.2) * 10);
        const gradient = `linear-gradient(45deg,
          rgb(${baseGray}, ${baseGray}, ${baseGray}) 0%,
          rgb(25, 25, 25) 100%)`;

        document.body.style.backgroundImage = gradient;
      } else {
        // Full complexity gradient for desktop
